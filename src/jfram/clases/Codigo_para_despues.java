/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

package jfram.clases;

import proyecto.clases.conexion.EmpleadoBD;

/**
 * 
 * @author Sammy Guergachi <sguergachi at gmail.com>
 */
public class Codigo_para_despues {
    /*EmpleadoBD ebd = new EmpleadoBD();

    public Codigo_para_despues() {
        ebd.abrirConexionSQL();
        actalizarLista();
    }
    
    
    private void actalizarLista() {
        for (int i = 0; i < ebd.listarEmpleados().size(); i++) {
            lista_empleados.addItem("No. Empleado: " + ebd.listarEmpleados().get(i).getId_empleado() + " Nombre: " + ebd.listarEmpleados().get(i).getNombre());
        }
    
    convertir un texto a entero
    Integer.parseInt(dato_num_empleado.getText())
    }*/
}
